...
export const processInsertDelete = (store, payload) => {
    ...
    if (!store.state.online.offline.usersData.lastUpdate) {
    ...
    return (
        prepareTable
            ...
            .then(() => window.database.insert(payload.table, payload.data.insert))
    );
};
...
export const updateUsersData = store => {
    ...
    let params = '';
    if (store.state.online.offline.usersData.lastUpdate) {
        params = `?lastUpdate=${store.state.online.offline.usersData.lastUpdate.toISOString()}`;
    }
