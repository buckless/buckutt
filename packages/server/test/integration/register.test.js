...
test('create a user with credentials provided', async t => {
    const user = await register(t.context.ctx, { firstname: 'Test', lastname: 'Test', mail: 'test@buckless.com' });
    ...
    await new t.context.factory.db.models.Wallet({ id: user.wallet.id }).destroy({ hardDelete: true });
    await t.context.factory.db.models.Membership.where({ user_id: user.id }).destroy({ hardDelete: true });
...
test('deny creation if the mail address is missing and no ticket provided', async t => {
    const error = await t.throwsAsync(register(t.context.ctx, { firstname: 'Test', lastname: 'Test' }));
...
test('create a user with credentials and card number provided', async t => {
    const user = await register(t.context.ctx, { firstname: 'Test', lastname: 'Test', mail: 'test@buckless.com', physicalId: 'P1' });
...
test('create a user with credentials and card number related to an anonymous wallet', async t => {
    ...
    const user = await register(t.context.ctx, { firstname: 'Test', lastname: 'Test', mail: 'test@buckless.com', physicalId: physical_id });
    ...
    await t.context.factory.db.models.Membership.where({ user_id: user.id }).save({ user_id: null, wallet_id: user.wallet.id }, { patch: true });
    await new t.context.factory.db.models.User({ id: user.id }).destroy({ hardDelete: true });
...
test('deny creation if the provided card doesn\'t exist', async t => {
    const error = await t.throwsAsync(register(t.context.ctx, { firstname: 'Error', lastname: 'Error', mail: 'error@buckless.com', physicalId: 'fakeId' }));
...
test('deny creation if the provided card already belongs to a user', async t => {
    const error = await t.throwsAsync(register(t.context.ctx, { firstname: 'Error', lastname: 'Error', mail: 'error@buckless.com', physicalId: t.context.cardtest.wallets[0].physical_id }));
...
test('create a user with a ticket number provided', async t => {
    ...
    await new t.context.factory.db.models.Ticket({ id: user.wallet.ticket.id }).save({ wallet_id: null });
    await new t.context.factory.db.models.Wallet({ id: user.wallet.id }).destroy({ hardDelete: true });
    await t.context.factory.db.models.Membership.where({ user_id: user.id }).destroy({ hardDelete: true });
    await new t.context.factory.db.models.User({ id: user.id }).destroy({ hardDelete: true });
...
test('create a user with a provided anonymous wallet related to the ticket number', async t => {
    const { physical_id, logical_id, ticket } = t.context.wallet2;
    const user = await register(t.context.ctx, { ticketNumber: ticket.physical_id, physicalId: physical_id });
...
test('deny creation if the provided ticket already belongs to a user', async t => {
    const error = await t.throwsAsync(register(t.context.ctx, { ticketNumber: t.context.tickettest.wallets[0].ticket.physical_id }));
...
test('create a user with a mail and a ticket number provided', async t => {
    ...
    const user = await register(t.context.ctx, { mail: 'test@buckless.com', ticketNumber: physical_id });
...
test('deny creation if the provided ticket already has a wallet and the provided card number doesn\'t match', async t => {
    const error = await t.throwsAsync(register(t.context.ctx, { ticketNumber: t.context.wallet2.ticket.physical_id, physicalId: t.context.wallet1.physical_id }));
    t.is(error.message, 'This ticket already belongs to another wallet');
