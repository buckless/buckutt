...
router.get('/services/usersData', async (req, res, next) => {
    ...
    try {
        ...
        const blockedCards_ = await models.MeanOfLogin.where({
            type: 'cardId',
            blocked: true
        })
            .fetchAll()
        ...
        const memberships = await models.Membership.where(
            'group_id',
            '!=',
        ...
        const ticketsMols = await models.MeanOfLogin.where('type', '=', 'ticketId')
            .andWhere(q => {
                q.where('deleted_at', '>=', lastUpdate);
                q.orWhere('updated_at', '>=', lastUpdate);
                q.orWhereHas('user', subQuery => {
        ...
        for (let i = ticketsMols.length - 1; i >= 0; i -= 1) {
            if (
                ticketsMols[i].deleted_at ||
                ticketsMols[i].blocked ||
                ticketsMols[i].user.deleted_at
            ) {
        ...
        // To convert someday with whereHas (+ find how to do the count with bookshelf)
        const pendingCardUpdates_ = await models.PendingCardUpdate.query(knex => {
            knex.select('pendingCardUpdates.*', 'meansoflogin.data', qb => {
                ...
                qb.andWhere(
                    'user_id',
                    '=',
                    bookshelf.knex.raw('??', ['pendingCardUpdates.user_id'])
                );
                qb.as('incrId');
            });
