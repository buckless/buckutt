    ...
    return transmit(wrap(0x5a, appId), 40)
        ...
        .then(data => {
            ...
            if (data.length !== 10 || data[data.length - 2] !== 0x91) {
                throw new Error(
                    `Error code: ${data.toString("hex")} in step keyId`
                );
            }
        ...
        .then(([data, RndA, msg]) => {
            ...
            if (data.length !== 10 || data[data.length - 2] !== 0x91) {
                throw new Error(
                    `Error code: ${data.toString(
                        "hex"
                    )} in step random key transmission`
                );
            }
        ...
        .then(data => {
            ...
            if (
                ...
                data[data.length - 2] !== 0x91
            ) {
                throw new Error(
                    `Error code: ${data.toString("hex")} in step fileI`
                );
            }
