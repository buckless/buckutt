...
export const logoutBuyer = store => {
    if (store.state.auth.buyer.isAuth) {
        ...
        return store
            ...
            .then(() => store.dispatch("loadDefaultItems"));
    }
...
export const buyerLogin = (store, { cardNumber, credit }) => {
    ...
    const params = `?buyer=${cardNumber.trim()}&molType=${
        config.buyerMeanOfLogin
    }`;
    const offlineAnswer = window.database
        .cardAccesses(cardNumber.trim())
        .then(memberships => ({
            data: {
                buyer: {
                    firstname: "Client",
    ...
    return (
        store
            ...
            .then(res => {
                const memberships = res.data.buyer.memberships.map(
                    membership => ({
                ...
                const buyerCredit =
                    typeof credit === "number" ? credit : res.data.buyer.credit;
